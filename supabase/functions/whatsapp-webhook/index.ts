...
function buildSystemPrompt(profile: UserProfileFields): string {
  ...
  const timeContext = profile.timezone
    ? `The user's timezone is ${profile.timezone}. Resolve all relative times ("tomorrow", "at 4", "in an hour", "4pm") against their local time given above, and always produce execution_time_iso as a proper ISO 8601 string carrying that timezone's UTC offset — never assume UTC. For a bare time-of-day with no AM/PM ("remind me at 10", "sleep at 4"), pick whichever of AM/PM is the nearest upcoming occurrence in their local time — that alone is NOT ambiguous enough to need clarification. But if the DATE itself is genuinely unclear even after applying "nearest upcoming occurrence" — e.g. "remind me next week" or "sometime soon" with no day, month, or specific occasion given — that IS ambiguous enough: set needs_clarification to true and ask which day/date they mean instead of silently picking one.`
...
If the message is genuinely ambiguous in a way that matters (e.g. "remind me to call him" with no indication who "him" is, "buy groceries" with no time when a time seems intended, or a reminder with no resolvable date per the timezone rules above), set needs_clarification to true and ask ONE tactical follow-up question in ai_response instead of guessing. Only do this when the ambiguity would actually change what gets saved or when the reminder fires — do not ask questions for things that are fine to leave general (e.g. "insight" or "document" style notes rarely need clarification), and do not ask about the date/time when a bare time-of-day was already resolved via nearest-upcoming-occurrence — that's expected behavior, not ambiguity.
